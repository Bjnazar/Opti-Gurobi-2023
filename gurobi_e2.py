...
# Constantes
num_camiones_electricos = 2
num_camiones_diesel = 2
num_destinos = 3
num_origenes = 3
num_dias = 10
bigM = 100**9
...
def agregar_restricciones(ls_activas):
    ...
    if 11 in ls_activas:
        model.addConstrs(
            (
                quicksum(
                    Z[i, b, t, d] for i in Camiones for b in Bloques for t in Dias
                )
                >= 1
                for d in Destinos
    ...
    # R16
    if 16 in ls_activas:
        model.addConstrs((bigM*(1-Y[i, b-Bo[i,o], t, o]) <= W[i, b, t, o]
        for i in Camiones 
